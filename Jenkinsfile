pipeline {
    ...
    environment {
        IMAGE_NAME = "10.67.60.223:30002/yotta/hello-world-python"
        IMAGE_TAG = "latest"
        HARBOR_CREDENTIALS_ID = "harbor-credentials"
        GITHUB_CREDENTIALS_ID = "github-credentials"
        ARGOCD_SERVER = "https://10.67.60.223:32007"
        ...
        stage('Login to Harbor') {
            steps {
                withCredentials([usernamePassword(credentialsId: HARBOR_CREDENTIALS_ID, usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
                    sh "echo $DOCKER_PASS | docker login 10.67.60.223:30002 -u $DOCKER_USER --password-stdin"
                }
            }
        }
